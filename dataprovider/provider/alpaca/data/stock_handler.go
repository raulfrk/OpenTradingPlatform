...
func handleAlpacaStockDataRequest(req requests.DataRequest) types.DataResponse {
	...
	switch dtype {
	case types.Bar:
		messages, response = handleDataFetch[marketdata.GetBarsRequest, marketdata.Bar, *sharedent.Bar](client.GetBars, symbol, marketdata.GetBarsRequest{
	...
	case types.Trades:
		messages, response = handleDataFetch[marketdata.GetTradesRequest, marketdata.Trade, *sharedent.Trade](client.GetTrades, symbol, marketdata.GetTradesRequest{
			Feed:      marketdata.IEX,
			PageLimit: 10000,
			Start:     time.Unix(req.GetStartTime(), 0),
			End:       time.Unix(req.GetEndTime(), 0),
	...
	case types.Quotes:
		messages, response = handleDataFetch[marketdata.GetQuotesRequest, marketdata.Quote, *sharedent.Quote](client.GetQuotes, symbol, marketdata.GetQuotesRequest{
			PageLimit: 10000,
			Start:     time.Unix(req.GetStartTime(), 0),
			End:       time.Unix(req.GetEndTime(), 0),
